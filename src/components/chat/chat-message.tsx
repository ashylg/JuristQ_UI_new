...
import { User, Scale, Zap, CircleDot, ChevronDown } from "lucide-react";
                ...
                <div className={cn("text-sm leading-relaxed text-foreground/90", isUser && "whitespace-pre-wrap")}>
                    {isUser ? (
                        ...
                        <ReactMarkdown
                            ...
                            components={{
                                // Custom Legal Headers
                                h3: ({ node, ...props }) => <h3 className="text-lg font-serif font-bold text-primary mt-6 mb-2 border-b border-border pb-1" {...props} />,
                                h4: ({ node, ...props }) => <h4 className="text-sm font-bold uppercase tracking-wide text-muted-foreground mt-4 mb-2" {...props} />,
                                // High-Contrast Legal Emphasis
                                strong: ({ node, ...props }) => <strong className="font-bold text-foreground bg-accent/10 px-0.5 rounded text-[0.95em]" {...props} />,
                                // Clean Lists
                                ul: ({ node, ...props }) => <ul className="list-disc pl-5 space-y-1 my-2" {...props} />,
                                ol: ({ node, ...props }) => <ol className="list-decimal pl-5 space-y-1 my-2" {...props} />,
                                // Legal Citation Highlighting (matches "s 22", "Act 1993", etc in plain text)
                    ...
                    <div className="flex items-center gap-2 pt-2 text-xs text-muted-foreground border-t border-border/30 mt-2">
                        <div className="flex items-center gap-1.5 bg-muted/50 px-2 py-1 rounded-md">
                            ...
                            <span className="ml-1 text-muted-foreground/60">
                                · ~${ai_metadata.estimated_cost.toFixed(4)}
                            </span>
                        </div>
