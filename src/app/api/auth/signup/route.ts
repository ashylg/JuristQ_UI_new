...
export async function POST(request: Request) {
  ...
  const upstream = await backendAuthFetch("/v1/auth/signup", {
    method: "POST",
  ...
  if (!upstream.ok) {
    return NextResponse.json({ error: data?.error || "Sign up failed", detail: data?.detail }, { status: upstream.status });
  }
